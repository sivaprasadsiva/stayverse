...
class Admin extends CI_model
{
  ...
  function vehicle_booking_view(){
    return $this->db->select('*,vm.id as mid,vt.id as tid,v.id as vid,l.location as loc')
                    ...
                    ->join('user_login u','u.id = b.user_id')
                    ->order_by('b.id','desc')
                    ->get_where('vehicle_booking b')->result_array();
  }
  function vehicle_type($data){
    return $this->db->insert('vehicle_type',$data);
  }
  function vehicle_type_update($data,$id){
    return $this->db->update('vehicle_type',$data,['id'=>$id]);
  }
  function vehicle_type_view(){
    return $this->db->get('vehicle_type')->result_array();
  }
  function vehicle_type_id($id){
    return $this->db->get_where('vehicle_type',['id'=>$id])->result_array();
  }
  function delete_vehicle_type($id){
    return $this->db->delete('vehicle_type',['id'=>$id]);
  }
  function vehicle_model($data){
    return $this->db->insert('vehicle_model',$data);
  }
  function vehicle_model_update($data,$id){
    return $this->db->update('vehicle_model',$data,['id'=>$id]);
  }
  function vehicle_model_view(){
    $this->db->select('*,vm.id as mid,vt.id as tid');
    $this->db->join('vehicle_type vt','vt.id = vm.type_id');
    return $this->db->get('vehicle_model vm')->result_array();
  }
  function vehicle_model_id($id){
    $this->db->select('*,vm.id as mid,vt.id as tid');
    $this->db->join('vehicle_type vt','vt.id = vm.type_id');
    return $this->db->get_where('vehicle_model vm',['vm.id'=>$id])->result_array();
  }
  function delete_vehicle_model($id){
    return $this->db->delete('vehicle_model',['id'=>$id]);
  }
  function vehicle($data){
    return $this->db->insert('vehicle',$data);
  }
  function vehicle_view(){
    $this->db->select('*,vm.id as mid,v.id as vid');
    $this->db->join('vehicle_model vm','vm.id = v.model_id');
    return $this->db->get('vehicle v')->result_array();
  }
  function company_name_ajax($id){
    return $this->db->get_where('vehicle_model',['type_id' => $id])->result_array();
  }
  ...
  function vehicle_id($id){
    $this->db->select('*,vm.id as mid,v.id as vid');
    $this->db->join('vehicle_model vm','vm.id = v.model_id');
    return $this->db->get_where('vehicle v',['v.id'=>$id])->result_array();
  }
  function vehicle_update($data,$id){
    return $this->db->update('vehicle',$data,['id'=>$id]);
  }
