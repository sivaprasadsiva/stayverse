...
class Dashboard_admin extends CI_Controller
{
    ...
    function vehicle_booking()
    {
        ...
        $data['role'] = $_SESSION['admin_o']['role'];
        $data['booking'] = $this->admin->vehicle_booking_view();
        $this->load->view('adminPanel/header', $data);
        $this->load->view('adminPanel/sidebar');
        $this->load->view('adminPanel/vehicle_booking');
        $this->load->view('adminPanel/footer');
    }
    function homestay(){
    ...
    function vehicle_type(){
        if(isset($_POST['submit'])){
            ...
            $this->admin->vehicle_type($data);
            redirect(base_url().'dashboard_admin/vehicle_type');
        }else{
            $data['role'] = $_SESSION['admin_o']['role'];
            $data['type'] = $this->admin->vehicle_type_view();
            $this->load->view('adminPanel/header',$data);
            $this->load->view('adminPanel/sidebar');
            $this->load->view('adminPanel/vehicletype');
            $this->load->view('adminPanel/footer');
        }
    }
    function vehicle_type_update($id){
        if(isset($_POST['submit'])){
            $data = [

                'type' => $_POST['type']
            ];
            $this->admin->vehicle_type_update($data,$id);
            redirect(base_url().'dashboard_admin/vehicle_type');
        }else{
            $data['role'] = $_SESSION['admin_o']['role'];
            $data['title'] = 'Vehicle Type Update';
            $type          = $this->admin->vehicle_type_id($id);
            $data['type']  = $type[0]['type'];
            $data['id']    = $type[0]['id'];
            $this->load->view('adminPanel/header',$data);
            $this->load->view('adminPanel/sidebar');
            $this->load->view('adminPanel/vehicletype_update');
            $this->load->view('adminPanel/footer');
        }
    }
    function vehicle_type_delete(){
        $this->admin->delete_vehicle_type($_POST['id']);
        redirect(base_url().'dashboard_admin/vehicle_type');
    }
    function vehicle_model(){
        if(isset($_POST['submit'])){
            ...
            $this->admin->vehicle_model($data);
            redirect(base_url().'dashboard_admin/vehicle_model');
        }else{
            $data['role'] = $_SESSION['admin_o']['role'];
            $data['type'] = $this->admin->vehicle_type_view();
            $data['model'] = $this->admin->vehicle_model_view();
            $this->load->view('adminPanel/header',$data);
            $this->load->view('adminPanel/sidebar');
            $this->load->view('adminPanel/vehiclemodel');
            $this->load->view('adminPanel/footer');
        }
    }
    function vehicle_model_update($id){
        if(isset($_POST['submit'])){
            $data = [
            ...
            $this->admin->vehicle_model_update($data,$id);
            redirect(base_url().'dashboard_admin/vehicle_model');
        }else{
            $data['role'] = $_SESSION['admin_o']['role'];
            $data['title']      = 'Vehicle Model Update';
            $data['type']       = $this->admin->vehicle_type_view();
            $model              = $this->admin->vehicle_model_id($id);
            $data['company_name'] = $model[0]['company_name'];
            $data['typeid']     = $model[0]['type_id'];
            $data['id']         = $id;
            $this->load->view('adminPanel/header',$data);
            $this->load->view('adminPanel/sidebar');
            $this->load->view('adminPanel/vehiclemodel_update');
            $this->load->view('adminPanel/footer');
        }
    }
    function vehicle_model_delete(){
        $this->admin->delete_vehicle_model($_POST['id']);
        redirect(base_url().'dashboard_admin/vehicle_model');
    }
    function vehicle(){
        ...
        if(isset($_POST['submit'])){
            ...
            $this->load->library('upload',$config);
            if($this->upload->do_upload('rcbook')){
                $rc_book = $this->upload->data();
                $rc      = $rc_book['file_name'];

                if($this->upload->do_upload('insurance')){
                    $insu = $this->upload->data();
                    $insurance = $insu['file_name'];
                    ...
                    $this->admin->vehicle($data);
                    redirect(base_url().'dashboard_admin/vehicle');
                }else{
                    echo $this->upload->display_errors();
                }
                }else{
            ...
            $data['role'] = $_SESSION['admin_o']['role'];
            $data['type']    = $this->admin->vehicle_type_view();
            $data['vehicle'] = $this->admin->vehicle_view();
            $this->load->view('adminPanel/header',$data);
            $this->load->view('adminPanel/sidebar');
            $this->load->view('adminPanel/vehicle');
            $this->load->view('adminPanel/footer');
        }
    }
    function vehicle_delete(){
        $this->admin->delete_vehicle($_POST['id']);
        redirect(base_url().'dashboard_admin/vehicle');
    }
    function vehicle_update($id){
        if(isset($_POST['submit'])){
         $data = [
                    ...
                    $this->admin->vehicle_update($data,$id);
                    redirect(base_url().'dashboard_admin/vehicle');
        }else{
            $vehicle = $this->admin->vehicle_id($id);
            $data['role']    = $_SESSION['admin_o']['role'];
            $data['title']   = 'Vehicle Update';
            $data['type']    = $this->admin->vehicle_type_view();
            $data['company_name'] = $this->admin->vehicle_model_view();
            $data['vehicle_name'] = $vehicle[0]['vehicle_name'];
            $data['model_id']     = $vehicle[0]['model_id'];
            $data['vehicle_number']= $vehicle[0]['vehicle_number'];
            $data['insurance']    = $vehicle[0]['insurance_exp_date'];
            $data['rental_price'] = $vehicle[0]['rental_price'];
            $data['typeid']       = $vehicle[0]['type_id'];
            $data['id']           = $id;
            $data['modelid']      = $vehicle[0]['model_id'];
            $data['img1']         = base_url('assets/images/').$vehicle[0]['vehicle_image'];
            $data['img2']         = base_url('assets/images/').$vehicle[0]['rcbook'];
            $data['img3']         = base_url('assets/images/').$vehicle[0]['insurance'];
            $this->load->view('adminPanel/header',$data);
            $this->load->view('adminPanel/sidebar');
            $this->load->view('adminPanel/vehicle_update');
            $this->load->view('adminPanel/footer');
        }
    }
    function update_vehicle_img1($id){
        if(isset($_POST['submit'])){
            $config = array(
                'upload_path' => './assets/images',
                'allowed_types'=> 'jpeg|png|jpg',
            ...
            if($this->upload->do_upload('image')){
                ...
                $this->admin->vehicle_update($data,$id);
                redirect(base_url().'dashboard_admin/vehicle_update/'.$id);
            }else{
                echo $this->upload->display_errors();
            }
        }else{
    ...
    function update_vehicle_img2($id){
        if(isset($_POST['submit'])){
            $config = array(
                'upload_path' => './assets/images',
                'allowed_types'=> 'jpeg|png|jpg',
    ...
    function update_vehicle_img3($id){
        if(isset($_POST['submit'])){
            $config = array(
                'upload_path' => './assets/images',
                'allowed_types'=> 'jpeg|png|jpg',
